...
-- To delete a partition, we need to create a dummy table that has the same structure as the original table and then swap out the partition from the original table to  the dummy table. 
...
-- 20240101 is actually the second partition since the first partition will correspond to all the values before 20240101, which, in our case, would be empty. 
...
-- Since the partition ranges should be exactly the same as TripTable, which now contains one extra 20240401 partition, we have to create another temporary table with all the four partition ranges as shown by copying the data from the TripTable_20240301 temporary table.
